...
# Test ALL models in parallel (default: 5 workers)
...
It now enriches `Write-Progress` with per-session state, attempt, PID, elapsed, and message details sourced from each model's `logs/instance-status.json`.
...
| `-MaxParallel` | int | `5` | Maximum simultaneous CLI instances (range: 1..8) |
...
# Two specific models only
.\Invoke-LLMOrchestrator.ps1 -SpecificModels @("gpt-4.1", "gpt-5.3-codex") -MaxParallel 2
...
```json
{
  "llmCategories": {
    ...
    "standard": {
      ...
      "multiplier": 1,
      "models": [
        { "name": "GPT-5.3-Codex", "model": "gpt-5.3-codex", "enabled": true, "costPerRequest": 0.0005 }
      ]
    }
  },
  "testConfiguration": {
